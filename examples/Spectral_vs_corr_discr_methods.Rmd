...
[pseudo](https://neurodatadesign.github.io/fngs/reveal/pdfs/spectr_vs_corr.pdf)
...
```{r, fig.height=6, fig.width=12, message=FALSE}
  ...
  distf <- div2dist(div)
  discr_sim=discr(rdf(distf, as.character(ids)))
  ...
  distance_plot <- ggplot(melt(distf), aes(x=Var1, y=Var2, fill=value)) + 
    ...
    xlab("Scan") + ylab("Scan") + ggtitle(paste('Spectral Approach, d=', discr_sim))
  kde_plot <- ggplot() +
  ...
  distance_plot <- ggplot(melt(Draw), aes(x=Var1, y=Var2, fill=value)) + 
    geom_tile() +
    scale_fill_gradientn(colours=c("darkblue","blue","purple","green","yellow"),name="distance") +
    xlab("Scan") + ylab("Scan") + ggtitle(paste('Correlational Approach, d=', discrraw))
  kde_plot <- ggplot() +
    geom_ribbon(data=meltkde, aes(x=distance, ymax=Probability, fill=Relationship), ymin=0, alpha=0.5) +
    ggtitle('Subject Relationships')
...
```{r, fig.height=6, fig.width=12, message=FALSE}
  ...
  discr_sims <- data.frame(cov=c(), discr_spectral=c(), discr_corr=c())
  ids <- ceiling(1:40/2)
  
  for (i in 1:length(cov_sim)) {
    discr_spec <- array(0, dim=c(10, 1))
    discr_raw <- array(0, dim=c(10, 1))
    for(j in 1:10) {
      signal <- sapply(seq(from=1, to=nsub*ntrial, by=1), function(id) {
          sub <- ceiling(id/ntrial)
      ...
      div <- freq2div(amp_sig)
      distf <- div2dist(div)
      discr_spec[j] = discr(rdf(distf, as.character(ids)))
      ...
      Draw <- distance(wgraphs)
      discr_raw[j] <- discr(rdf(Draw, ids))
    }
    
    
    discr_sims <- rbind(discr_sims, data.frame(cov=cov_sim[i], discr_spectral=mean(discr_spec), discr_corr=mean(discr_raw)))
  }

  discr_noise_sim_plot <- ggplot(data = melt(discr_sims, id.vars="cov"), aes(x=cov, y=value, colour=variable, group=variable)) +
    geom_line() +
